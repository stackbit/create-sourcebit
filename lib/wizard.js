...
class Wizard {
    constructor(parameters) {
        this.debug = debug('wizard');

        this.pluginRegistry = pluginRegistry;

        if (typeof parameters.plugins === 'string') {
            try {
                const localPluginRegistry = require(path.resolve(process.cwd(), parameters.plugins));

                if (!Array.isArray(localPluginRegistry)) {
                    throw new Error('Plugin registry must be an array');
                }

                this.pluginRegistry = this.pluginRegistry.concat(localPluginRegistry);
            } catch (error) {
                ora('Could not load local plugin registry file. Is the path correct? Is it well-formatted?').fail();
...
${chalk.dim(`v${pkg.version}`)}
    ...
    installPlugins(pluginNames) {
        const installablePluginNames = pluginNames.filter((pluginName) => {
            return pluginName && pluginName[0] !== path.sep && pluginName[0] !== '.';
        });
    ...
    loadPlugins(pluginNames) {
        ...
        pluginNames.forEach((pluginName) => {
            ...
            const requirePath =
                pluginName[0] === path.sep || pluginName[0] === '.' ? pluginName : path.join(process.cwd(), 'node_modules', pluginName);
    ...
    async start({ currentConfig }) {
        ...
        try {
            const pluginsByType = this.pluginRegistry.reduce((result, item) => {
            ...
            const questions = [
                {
                    when: Boolean(pluginsByType.source),
                    type: 'checkbox',
                    name: 'stage1Plugins',
                    message: `Select the source plugins. ${chalk.reset.dim('They define the sources of your data.')}`,
                    choices: pluginsByType.source || [],
                    validate: (value) => (value.length > 0 ? true : 'You must select at least one source plugin.')
                },
                {
                    when: Boolean(pluginsByType.transform),
                    type: 'checkbox',
                    name: 'stage2Plugins',
                ...
                {
                    when: Boolean(pluginsByType.target),
                    type: 'checkbox',
                    name: 'stage3Plugins',
            ...
            const allPlugins = stage1Plugins.concat(stage2Plugins).concat(stage3Plugins).filter(Boolean);
            ...
            allPlugins.forEach((pluginName, pluginIndex, plugins) => {
                setupData = setupData.then(async (setupData) => {
                    ...
                    const { getOptionsFromSetup, getSetup, options: optionsSchema = {} } = pluginModules[pluginName];
                    const pluginDebug = debug(`plugin:${pluginName}`);

                    if (typeof getOptionsFromSetup !== 'function' || typeof getSetup !== 'function') {
                        return;
                    }

                    const currentPluginBlock = (currentConfig.plugins || []).find(
                        (plugin) => plugin.module && plugin.module.name === pluginModules[pluginName].name
                    );
                    const currentOptions = (currentPluginBlock && currentPluginBlock.options) || {};
                    ...
                    try {
                        ...
                        Object.keys(pluginOptions).forEach((key) => {
                            ...
                            if (schema.env) {
                                ...
                                if (pluginOptions[key] === undefined || schema.private) {
                                    publicOptions[key] = new WrappedFunction(`process.env['${schema.env}']`);
                                } else {
                                    publicOptions[key] = new WrappedFunction(
                                        `process.env['${schema.env}'] || ${JSON.stringify(pluginOptions[key])}`
                                    );
                                }
                            } else {
                                if (!schema.private) {
                        ...
                        ora(`Plugin ${chalk.bold(pluginName)} could not be configured due to a fatal error.`).fail();
